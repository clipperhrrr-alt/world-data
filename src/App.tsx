...
import {
  ...
  BarChart3, Eye, Hash, Clock, Activity
} from 'lucide-react';
...
// Types
interface Category { id: string; name: string; icon: React.ElementType; description: string; }
interface Article {
  id: string; title: string; content: string; summary: string; category: string;
  source: string; publishedAt: string; tags: string[]; imageUrl: string; sourceUrl: string; imageSearchUrl: string;
}
interface SearchResult { query: string; content: string; timestamp: string; }
...
const buildArticle = (raw: any, category: string, index: number): Article => {
  ...
  return {
    id: `${category}-${index}-${Date.now()}`, title, content: raw.summary || '', summary: raw.summary || '',
    category: category.charAt(0).toUpperCase() + category.slice(1), source: 'World Data AI', publishedAt: 'Just now',
    tags: raw.tags || [category], imageUrl: `https://loremflickr.com/800/400/${seed}`,
    sourceUrl: `https://www.google.com/search?q=${encoded}`, imageSearchUrl: `https://www.google.com/search?q=${encoded}&tbm=isch`,
  };
};
...
const fetchCategoryData = async (category: string): Promise<Article[]> => {
  ...
  try {
    ...
    if (jsonMatch) { return JSON.parse(jsonMatch[0]).map((a: any, i: number) => buildArticle(a, category, i)); }
    return [buildArticle({ title: `Latest ${category} updates`, summary: response.slice(0, 200) }, category, 0)];
  } catch (error) { console.error('Error fetching category data:', error); return []; }
};
...
// Bookmark helpers
const getSaved = (): Article[] => { try { return JSON.parse(localStorage.getItem('worldDataSaved') || '[]'); } catch { return []; } };
const setSaved = (articles: Article[]) => localStorage.setItem('worldDataSaved', JSON.stringify(articles));

// Analytics tracking — localStorage based, no backend
const trackEvent = (type: 'view' | 'search' | 'bookmark' | 'article_load', data: string) => {
  try {
    const key = 'worldDataAnalytics';
    const stats = JSON.parse(localStorage.getItem(key) || '{"views":{},"searches":[],"articleLoads":0,"bookmarks":0,"activity":[]}');
    if (type === 'view') stats.views[data] = (stats.views[data] || 0) + 1;
    if (type === 'search') stats.searches.unshift({ query: data, time: new Date().toISOString() });
    if (type === 'search' && stats.searches.length > 20) stats.searches = stats.searches.slice(0, 20);
    if (type === 'article_load') stats.articleLoads += 1;
    if (type === 'bookmark') stats.bookmarks += 1;
    stats.activity.unshift({ type, data, time: new Date().toISOString() });
    if (stats.activity.length > 15) stats.activity = stats.activity.slice(0, 15);
    localStorage.setItem(key, JSON.stringify(stats));
  } catch {}
};

const getAnalytics = () => {
  try { return JSON.parse(localStorage.getItem('worldDataAnalytics') || '{"views":{},"searches":[],"articleLoads":0,"bookmarks":0,"activity":[]}'); }
  catch { return { views: {}, searches: [], articleLoads: 0, bookmarks: 0, activity: [] }; }
};

// Logo
const Logo = ({ size = 'normal' }: { size?: 'small' | 'normal' | 'large' }) => {
  const isLarge = size === 'large'; const isSmall = size === 'small';
  return (
...
const ThemeToggle = () => {
  ...
  return (
    <button onClick={toggleTheme} className={`relative w-12 h-6 rounded-full p-1 transition-colors ${theme === 'dark' ? 'bg-slate-700' : 'bg-slate-300'}`}>
      <motion.div animate={{ x: theme === 'dark' ? 24 : 0 }} transition={{ type: 'spring', stiffness: 500, damping: 30 }} className="w-4 h-4 rounded-full bg-gradient-to-r from-blue-500 to-cyan-400 flex items-center justify-center">
...
const Navbar = ({ onGetStarted }: { onGetStarted: () => void }) => {
  ...
  return (
    <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? (theme === 'dark' ? 'bg-slate-900/95 backdrop-blur-md border-b border-slate-800' : 'bg-white/95 backdrop-blur-md border-b border-slate-200') : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <Logo />
          <div className="hidden lg:flex items-center gap-8">
            {['Home', 'Categories', 'Dashboard', 'Analytics', 'About'].map((link) => (
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className={`lg:hidden p-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{isMobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}</button>
        </div>
      </div>
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className={`lg:hidden border-b ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            <div className="px-6 py-6 space-y-4">
              ...
              <div className="pt-4 space-y-3"><div className="flex items-center gap-3"><span className={theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}>Theme</span><ThemeToggle /></div>
          ...
          <motion.div variants={fadeInUp} className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className={`p-4 rounded-xl ${theme === 'dark' ? 'bg-slate-800/50' : 'bg-white'} border ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                <stat.icon className="w-6 h-6 mx-auto mb-2 text-blue-500" /><p className="text-2xl font-bold text-blue-500">{stat.value}</p><p className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>{stat.label}</p>
              </div>
            ))}
          </motion.div>
        </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((category, i) => (
            <motion.button key={category.id} initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1 }} onClick={() => onSelect(category)}
...
const ArticleCard = ({ article, isSaved, onBookmark }: { article: Article; isSaved: boolean; onBookmark: (a: Article) => void }) => {
  const { theme } = useTheme();
  return (
    <motion.article initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`rounded-2xl overflow-hidden transition-all hover:scale-[1.02] ${theme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
      <div className="relative h-40 overflow-hidden bg-slate-700">
        <img src={article.imageUrl} alt={article.title} className="w-full h-full object-cover" loading="lazy" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => onBookmark(article)} className={`transition-colors ${isSaved ? 'text-blue-500' : theme === 'dark' ? 'text-slate-500 hover:text-blue-400' : 'text-slate-400 hover:text-blue-500'}`}><Bookmark className="w-4 h-4" fill={isSaved ? 'currentColor' : 'none'} /></button>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">{article.tags.map((tag) => (<span key={tag} className={`text-xs px-2 py-1 rounded ${theme === 'dark' ? 'bg-slate-700 text-slate-400' : 'bg-slate-100 text-slate-600'}`}>#{tag}</span>))}</div>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const cat = activeTab === 'trending' ? 'trending' : activeTab === 'tech' ? 'tech' : activeTab;
    loadArticles(cat);
    trackEvent('view', cat);
  }, [activeTab]);
  ...
  const handleBookmark = (article: Article) => {
    ...
    if (!exists) trackEvent('bookmark', article.title);
  };
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {activeTab !== 'saved' && (
              <button onClick={() => loadArticles(activeTab === 'trending' ? 'trending' : activeTab)} disabled={loading} className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800 text-white font-semibold hover:bg-slate-700 transition-colors disabled:opacity-50"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /> Refresh</button>
            )}
          </div>
        </motion.div>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition-all ${activeTab === tab.id ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white' : theme === 'dark' ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-white text-slate-600 border border-slate-200'}`}>
        ...
        {loading ? (<div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 text-blue-500 animate-spin" /><span className="ml-3 text-slate-400">Loading fresh data...</span></div>)
        : displayArticles.length === 0 ? (<div className="text-center py-20"><p className={theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}>{activeTab === 'saved' ? 'No saved articles yet. Bookmark articles to see them here.' : 'No articles found. Try refreshing.'}</p></div>)
        : (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{displayArticles.map((article) => (<ArticleCard key={article.id} article={article} isSaved={isArticleSaved(article.id)} onBookmark={handleBookmark} />))}</div>)}
      </div>
    </section>
  );
};

// Analytics Dashboard — stat tiles, bar chart, top searches, activity feed
const Analytics = () => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });
  const { theme } = useTheme();
  ...
  useEffect(() => {
    const update = () => { setAnalytics(getAnalytics()); setSavedCount(getSaved().length); };
    update();
    const interval = setInterval(update, 2000); // refresh every 2s for live feel
    return () => clearInterval(interval);
  }, []);

  const totalViews = Object.values(analytics.views).reduce((a: number, b: number) => a + b, 0) as number;
  const totalSearches = analytics.searches.length;
  const categoriesExplored = Object.keys(analytics.views).length;

  const viewEntries = Object.entries(analytics.views) as [string, number][];
  const maxViews = Math.max(...viewEntries.map(([, v]) => v), 1);

  const stats = [
    { label: 'Total Views', value: totalViews, icon: Eye, color: 'text-blue-500' },
    { label: 'Articles Loaded', value: analytics.articleLoads, icon: Activity, color: 'text-cyan-500' },
    { label: 'AI Searches', value: totalSearches, icon: Search, color: 'text-purple-500' },
    { label: 'Bookmarks', value: savedCount, icon: Bookmark, color: 'text-amber-500' },
  ];

  const recentActivity = analytics.activity.slice(0, 8);

  return (
    <section ref={ref} id="analytics" className={`py-24 ${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} className="mb-10">
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-500/10 text-purple-500 text-sm font-semibold mb-4"><BarChart3 className="w-4 h-4" /> Analytics Dashboard</span>
          <h2 className={`text-4xl font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Your Activity Stats</h2>
          <p className={`mt-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Track your searches, views, and engagement — all stored locally on your device.</p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {stats.map((stat, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1 }}
              className={`p-6 rounded-2xl ${theme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
              <stat.icon className={`w-8 h-8 mb-3 ${stat.color}`} />
              <p className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{stat.value}</p>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          {/* Category views bar chart — pure CSS, no chart library */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} className={`p-6 rounded-2xl ${theme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
            <h3 className={`text-lg font-bold mb-6 flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}><BarChart3 className="w-5 h-5 text-blue-500" /> Category Views</h3>
            {viewEntries.length === 0 ? (
              <p className={theme === 'dark' ? 'text-slate-400 text-sm' : 'text-slate-600 text-sm'}>No views yet. Start exploring categories!</p>
            ) : (
              <div className="space-y-4">
                {viewEntries.map(([cat, count]) => (
                  <div key={cat}>
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-sm font-medium capitalize ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>{cat}</span>
                      <span className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>{count}</span>
                    </div>
                    <div className={`h-2 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-slate-700' : 'bg-slate-100'}`}>
                      <motion.div initial={{ width: 0 }} animate={{ width: `${(count / maxViews) * 100}%` }} transition={{ duration: 0.8, ease: 'easeOut' }}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} className={`p-6 rounded-2xl ${theme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
            <h3 className={`text-lg font-bold mb-6 flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}><Hash className="w-5 h-5 text-blue-500" /> Top Searches & Activity</h3>
            {totalSearches === 0 && recentActivity.length === 0 ? (
              <p className={theme === 'dark' ? 'text-slate-400 text-sm' : 'text-slate-600 text-sm'}>No searches yet. Ask AI a question in any category!</p>
            ) : (
              <div className="space-y-3">
                {analytics.searches.slice(0, 5).map((s: any, i: number) => (
                  <div key={i} className="flex items-center justify-between">
                    <span className={`text-sm flex items-center gap-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}><Search className="w-3 h-3 text-blue-500" /> {s.query}</span>
                    <span className={`text-xs ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>{new Date(s.time).toLocaleDateString()}</span>
                  </div>
                ))}
                <div className={`pt-4 mt-2 border-t ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                  <p className={`text-xs font-semibold mb-3 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Recent Activity</p>
                  {recentActivity.map((a: any, i: number) => (
                    <div key={i} className="flex items-center gap-2 mb-2">
                      {a.type === 'view' && <Eye className="w-3 h-3 text-blue-500" />}
                      {a.type === 'search' && <Search className="w-3 h-3 text-purple-500" />}
                      {a.type === 'bookmark' && <Bookmark className="w-3 h-3 text-amber-500" />}
                      {a.type === 'article_load' && <Activity className="w-3 h-3 text-cyan-500" />}
                      <span className={`text-xs truncate ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>{a.data}</span>
                      <span className={`text-xs ml-auto ${theme === 'dark' ? 'text-slate-600' : 'text-slate-400'}`}><Clock className="w-3 h-3 inline" /></span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* Categories explored indicator */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} className={`mt-6 p-6 rounded-2xl ${theme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Categories Explored</h3>
            <span className="text-2xl font-bold text-blue-500">{categoriesExplored}/6</span>
          </div>
          <div className={`h-3 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-slate-700' : 'bg-slate-100'}`}>
            <motion.div initial={{ width: 0 }} animate={{ width: `${(categoriesExplored / 6) * 100}%` }} transition={{ duration: 0.8 }}
              className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-400" />
          </div>
        </motion.div>
      </div>
    ...
    <div className={`min-h-screen pt-24 pb-12 ${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className={`relative max-w-3xl mb-10 p-2 rounded-2xl flex items-center gap-2 ${theme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} placeholder={`Ask anything about ${category.name}...`} className={`flex-1 bg-transparent px-3 py-3 outline-none ${theme === 'dark' ? 'text-white placeholder:text-slate-500' : 'text-slate-900 placeholder:text-slate-400'}`} />
          <button onClick={handleSearch} disabled={isSearching || !searchQuery.trim()} className="px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-400 text-white font-semibold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center gap-2">{isSearching ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}{isSearching ? 'Searching...' : 'Ask AI'}</button>
        </div>
        {searchResult && (
        ...
        {loading ? (<div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 text-blue-500 animate-spin" /><span className="ml-3 text-slate-400">Loading articles...</span></div>)
        : articles.length === 0 ? (<p className={theme === 'dark' ? 'text-slate-400 text-center py-20' : 'text-slate-600 text-center py-20'}>No articles found.</p>)
        : (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{articles.map((article) => (<ArticleCard key={article.id} article={article} isSaved={isArticleSaved(article.id)} onBookmark={handleBookmark} />))}</div>)}
      </div>
    </div>
  );
};
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={isInView ? { opacity: 1, x: 0 } : {}}>
            ...
            <p className={`mb-6 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>World Data leverages AionLabs AI to deliver real-time, intelligent insights across gaming, technology, global affairs, viral trends, and more. Our mission is to democratize access to information through advanced AI curation — from major world events to the smallest trending reel.</p>
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={(e) => e.stopPropagation()} className={`relative w-full max-w-2xl max-h-[80vh] overflow-y-auto rounded-2xl p-8 ${theme === 'dark' ? 'bg-slate-900 border border-slate-700' : 'bg-white border border-slate-200'}`}>
          ...
          <div className={`space-y-4 text-sm ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
            {isPrivacy ? (
              <>
                <section><h3 className={`font-bold mb-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>1. Data Collection</h3><p>We do not store your search queries permanently. All searches are processed through AionLabs API in real-time. Analytics data is stored locally on your device only.</p></section>
                ...
                <section><h3 className={`font-bold mb-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>4. Analytics</h3><p>All analytics data (views, searches, bookmarks) is stored in your browser's localStorage. No data is sent to any server.</p></section>
              </>
            ) : (
              <>
    ...
    <footer className={`py-12 border-t ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          <div className="col-span-2 md:col-span-1"><Logo size="small" /><p className={`mt-4 text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>AI-powered global intelligence platform.</p></div>
          {[
            { title: 'Product', links: [{ label: 'Categories', action: () => scrollTo('categories') }, { label: 'Dashboard', action: () => scrollTo('dashboard') }, { label: 'Analytics', action: () => scrollTo('analytics') }, { label: 'API', action: () => window.open('https://www.aionlabs.ai/docs/', '_blank') }] },
            ...
            <div key={i}>
              ...
              <ul className="space-y-2">{col.links.map((link) => (<li key={link.label}><button onClick={link.action} className={`text-sm transition-colors ${theme === 'dark' ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900'}`}>{link.label}</button></li>))}</ul>
            </div>
          ))}
        </div>
...
// Main App
function App() {
  const [theme, setTheme] = useState<'dark' | 'light'>(() => { try { return localStorage.getItem('worldDataTheme') as 'dark' | 'light' || 'dark'; } catch { return 'dark'; } });
  const [currentView, setCurrentView] = useState<'home' | 'category'>('home');
  const [selectedCategory, setSelectedCategory] = useState<Category | null>(null);
  const [showTerms, setShowTerms] = useState(false);
  const [showPrivacy, setShowPrivacy] = useState(false);
  const toggleTheme = () => setTheme(prev => { const next = prev === 'dark' ? 'light' : 'dark'; try { localStorage.setItem('worldDataTheme', next); } catch {} return next; });
  return (
